...
static void
ImDrawList_PathBezierOffset(ImDrawList *drawList, float offset, const ImVec2 &p0, const ImVec2 &p1, const ImVec2 &p2,
                            const ImVec2 &p3) {
    using namespace ed;
    ...
    if (fill) {
        ...
        if (startArrowSize > 0.0f) {
            const auto start_dir = ImNormalized(
                    startDirHint ? *startDirHint : ImCubicBezierTangent(curve.P0, curve.P1, curve.P2, curve.P3, 0.0f));
        ...
        if (endArrowSize > 0.0f) {
            const auto end_dir = ImNormalized(
                    endDirHint ? -*endDirHint : ImCubicBezierTangent(curve.P0, curve.P1, curve.P2, curve.P3, 1.0f));
...
void ed::Link::Draw(ImDrawList *drawList, ImU32 color, float extraThickness) const {
    ...
    ImDrawList_AddBezierWithArrows(drawList, curve, m_Thickness + extraThickness,
                                   m_StartPin && m_StartPin->m_ArrowSize > 0.0f ? m_StartPin->m_ArrowSize +
                                                                                  extraThickness : 0.0f,
                                   m_StartPin && m_StartPin->m_ArrowWidth > 0.0f ? m_StartPin->m_ArrowWidth +
                                                                                   extraThickness : 0.0f,
                                   m_EndPin && m_EndPin->m_ArrowSize > 0.0f ? m_EndPin->m_ArrowSize + extraThickness
                                                                            : 0.0f,
                                   m_EndPin && m_EndPin->m_ArrowWidth > 0.0f ? m_EndPin->m_ArrowWidth + extraThickness
                                                                             : 0.0f,
                                   true, color, 1.0f,
...
ed::EditorContext::EditorContext(const ax::NodeEditor::Config *config)
        : m_Config(config), m_EditorActiveId(0), m_IsFirstFrame(true), m_IsFocused(false), m_IsHovered(false),
          ...
          m_HintBuilder(this), m_CurrentAction(nullptr), m_NavigateAction(this, m_Canvas), m_SizeAction(this),
          m_DragAction(this), m_SelectAction(this), m_ContextMenuAction(this), m_ShortcutAction(this),
          m_CreateItemAction(this), m_DeleteItemsAction(this), m_AnimationControllers{&m_FlowAnimationController},
          m_FlowAnimationController(this), m_HoveredNode(0), m_HoveredPin(0), m_HoveredLink(0), m_DoubleClickedNode(0),
          m_DoubleClickedPin(0), m_DoubleClickedLink(0), m_BackgroundClickButtonIndex(-1),
          m_BackgroundDoubleClickButtonIndex(-1), m_IsInitialized(false), m_Settings(), m_DrawList(nullptr),
          m_ExternalChannel(0) {
}
...
void ed::EditorContext::Begin(const char *id, const ImVec2 &size) {
    ...
    if (!ImRect_IsEmpty(previousVisibleRect) && previousSize != canvasSize) {
        ...
        auto currentAspectRatio = currentVisibleRect.GetHeight() ? (currentVisibleRect.GetWidth() /
                                                                    currentVisibleRect.GetHeight()) : 0.0f;
...
void ed::EditorContext::End() {
    ...
    // Highlight selected objects
    {
        ...
        for (auto &link: m_Links) {
            ...
            auto isLinkHighlighted =
                    isLinkHighlightedForPin(*link->m_StartPin) || isLinkHighlightedForPin(*link->m_EndPin);
    ...
    if (sortGroups ||
        ((m_Settings.m_DirtyReason & (SaveReasonFlags::Position | SaveReasonFlags::Size)) != SaveReasonFlags::None)) {
        ...
        std::sort(m_Nodes.begin(), groupsItEnd, [this](Node *lhs, Node *rhs) {
            const auto &lhsSize = lhs == m_SizeAction.m_SizedNode ? m_SizeAction.GetStartGroupBounds().GetSize()
                                                                  : lhs->m_GroupBounds.GetSize();
            const auto &rhsSize = rhs == m_SizeAction.m_SizedNode ? m_SizeAction.GetStartGroupBounds().GetSize()
...
# if 1
    ...
    {
        // Copy group nodes
        auto liveNodeCount = static_cast<int>(std::count_if(m_Nodes.begin(), m_Nodes.end(),
                                                            [](Node *node) { return node->m_IsLive; }));

        // Reserve two additional channels for sorted list of channels
        auto nodeChannelCount = m_DrawList->_Splitter._Count;
        ImDrawList_ChannelsGrow(m_DrawList,
                                m_DrawList->_Splitter._Count + c_ChannelsPerNode * liveNodeCount + c_LinkChannelCount);
...
void ed::EditorContext::UpdateAnimations() {
    ...
    for (auto animation: m_LastLiveAnimations) {
        const bool isLive = (std::find(m_LiveAnimations.begin(), m_LiveAnimations.end(), animation) !=
...
ed::Control ed::EditorContext::BuildControl(bool allowOffscreen) {
    ...
    auto checkInteractionsInArea = [this, &emitInteractiveArea, &hotObject, &activeObject, &clickedObject, &doubleClickedObject](
            ObjectId id, const ImRect &rect, Object *object) {
...
bool ed::Settings::Parse(const std::string &string, Settings &settings) {
    ...
    if (viewValue.is_object()) {
        ...
        if (!viewValue.contains("visible_rect") ||
            !tryParseVector(viewValue["visible_rect"]["min"], result.m_VisibleRect.Min) ||
            !tryParseVector(viewValue["visible_rect"]["max"], result.m_VisibleRect.Max))
...
ImVec2 ed::FlowAnimation::SamplePath(float distance) const {
    ...
    auto endPointIt = std::find_if(m_Path.begin(), m_Path.end(),
                                   [distance](const CurvePoint &p) { return distance < p.Distance; });
...
const float ed::NavigateAction::s_DefaultZoomLevels[] =
        {
                0.1f, 0.15f, 0.20f, 0.25f, 0.33f, 0.5f, 0.75f, 1.0f, 1.25f, 1.50f, 2.0f, 2.5f, 3.0f, 4.0f, 5.0f, 6.0f,
                7.0f, 8.0f
        };
...
ed::NavigateAction::NavigateAction(EditorContext *editor, ImGuiEx::Canvas &canvas) :
        ...
        m_ZoomLevels(editor->GetConfig().CustomZoomLevels.Size > 0 ? editor->GetConfig().CustomZoomLevels.Data
                                                                   : s_DefaultZoomLevels),
        m_ZoomLevelCount(editor->GetConfig().CustomZoomLevels.Size > 0 ? editor->GetConfig().CustomZoomLevels.Size
...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(const Control &control) {
    ...
    if (Editor->CanAcceptUserInput() && ImGui::IsKeyPressed(GetKeyIndexForF()) && Editor->AreShortcutsEnabled()) {
        ...
        if (!Editor->GetSelectedObjects().empty()) {
            if (m_Reason != NavigationReason::Selection || m_LastSelectionId != Editor->GetSelectionId() ||
        ...
        if (navigateToContent)
            NavigateTo(Editor->GetContentBounds(), NavigateAction::ZoomMode::WithMargin, -1.0f,
...
bool ed::SizeAction::Process(const Control &control) {
    if (m_Clean) {
        ...
        if (m_SizedNode->m_Bounds.GetSize() != m_StartBounds.GetSize() ||
            m_SizedNode->m_GroupBounds.GetSize() != m_StartGroupBounds.GetSize())
    ...
    if (control.ActiveNode == m_SizedNode) {
        const auto dragOffset = (control.ActiveNode == m_SizedNode) ? ImGui::GetMouseDragDelta(0, 0.0f)
                                                                    : m_LastDragOffset;
...
ed::EditorAction::AcceptResult ed::SelectAction::Accept(const Control &control) {
    ...
    if (Editor->CanAcceptUserInput() && control.BackgroundHot &&
        ...
        Object *clickedObject = control.ClickedNode ? static_cast<Object *>(control.ClickedNode)
        ...
        if (candidateAction != Paste && candidateAction != CreateNode) {
            ...
            if (!hasAnyLinks && m_Context.size() > 1) // one node cannot make connection to anything
            {
                ...
                links.erase(std::remove_if(links.begin(), links.end(), [&isNodeInContext](Link *link) {
                    return !isNodeInContext(link->m_StartPin->m_Node->m_ID) ||
                           !isNodeInContext(link->m_EndPin->m_Node->m_ID);
                }), links.end());
