    ...
    if (g.DragDropActive && (flags & ImGuiButtonFlags_PressedOnDragDropHold) &&
        ...
        if (IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem)) {
            ...
            if (CalcTypematicRepeatAmount(g.HoveredIdTimer + 0.0001f - g.IO.DeltaTime, g.HoveredIdTimer + 0.0001f,
                                          DRAGDROP_HOLD_TO_OPEN_TIMER, 0.00f)) {
                pressed = true;
...
bool ImGui::ScrollbarEx(const ImRect &bb_frame, ImGuiID id, ImGuiAxis axis, float *p_scroll_v, float size_avail_v,
                        float size_contents_v, ImDrawFlags flags) {
    ImGuiContext &g = *GImGui;
    ImGuiWindow *window = g.CurrentWindow;
    if (window->SkipItems)
        return false;
...
bool ImGui::Checkbox(const char *label, bool *v) {
    ...
    if (!ItemAdd(total_bb, id)) {
        IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.LastItemStatusFlags | ImGuiItemStatusFlags_Checkable |
...
bool ImGui::BeginComboPreview() {
    ...
    if (window->SkipItems || !window->ClipRect.Overlaps(
            window->DC.LastItemRect)) // FIXME: Because we don't have a ImGuiItemStatusFlags_Visible flag to test last ItemAdd() result
        return false;
    IM_ASSERT(window->DC.LastItemRect.Min.x == preview_data->PreviewRect.Min.x && window->DC.LastItemRect.Min.y ==
...
bool ImGui::DragScalar(const char *label, ImGuiDataType data_type, void *p_data, float v_speed, const void *p_min,
    ...
    if (!temp_input_is_active) {
        const bool focus_requested =
                temp_input_allowed && (window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_Focused) != 0;
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char *label, const char *label_end) {
    ...
    if (!item_add) {
        ...
        IMGUI_TEST_ENGINE_ITEM_INFO(window->DC.LastItemId, label,
                                    window->DC.LastItemStatusFlags | (is_leaf ? 0 : ImGuiItemStatusFlags_Openable) |
...
bool ImGui::BeginMenu(const char *label, bool enabled) {
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        RenderArrow(window->DrawList,
                    pos + ImVec2(window->DC.MenuColumns.Offsets[1] + extra_w + g.FontSize * 0.30f, 0.0f), text_col,
                    ImGuiDir_Right);
    }
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.LastItemStatusFlags | ImGuiItemStatusFlags_Openable |
...
bool ImGui::MenuItem(const char *label, const char *shortcut, bool selected, bool enabled) {
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        if (selected)
            RenderCheckMark(window->DrawList, pos +
                                              ImVec2(window->DC.MenuColumns.Offsets[1] + extra_w + g.FontSize * 0.40f,
