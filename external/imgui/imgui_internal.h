...
#include <cstdlib>     // NULL, malloc, free, qsort, atoi, atof
...
enum ImGuiItemFlags_ {
    ...
    ImGuiItemFlags_Disabled = 1
            << 2,  // false    // [BETA] Disable interactions but doesn't affect visuals yet. See github.com/ocornut/imgui/issues/211
...
enum ImGuiItemAddFlags_ {
    ...
    ImGuiItemAddFlags_Focusable = 1
            << 0    // FIXME-NAV: In current/legacy scheme, Focusable+TabStop support are opt-in by widgets. We will transition it toward being opt-out, so this flag is expected to eventually disappear.
};
...
enum ImGuiNavMoveFlags_ {
    ...
    ImGuiNavMoveFlags_AlsoScoreVisibleSet = 1
            << 5,   // Store alternate result in NavMoveResultLocalVisibleSet that only comprise elements that are already fully visible.
...
struct ImGuiContext {
    ...
    explicit ImGuiContext(ImFontAtlas *shared_font_atlas) {
        Initialized = false;
...
namespace ImGui {
    ...
    IMGUI_API void Shutdown(
            ImGuiContext *context);    // Since 1.60 this is a _private_ function. You can call DestroyContext() to destroy the context created by CreateContext().
    ...
    IMGUI_API void
    SetLastItemData(ImGuiWindow *window, ImGuiID item_id, ImGuiItemStatusFlags status_flags, const ImRect &item_rect);
    ...
    IMGUI_API void
    NavMoveRequestForward(ImGuiDir move_dir, ImGuiDir clip_dir, const ImRect &bb_rel, ImGuiNavMoveFlags move_flags);
    ...
    IMGUI_API ImVec2
    GetNavInputAmount2d(ImGuiNavDirSourceFlags dir_sources, ImGuiInputReadMode mode, float slow_factor = 0.0f,
    ...
    IMGUI_API void
    RenderMouseCursor(ImDrawList *draw_list, ImVec2 pos, float scale, ImGuiMouseCursor mouse_cursor, ImU32 col_fill,
    ...
    IMGUI_API bool
    ScrollbarEx(const ImRect &bb, ImGuiID id, ImGuiAxis axis, float *p_scroll_v, float avail_v, float contents_v,
                ImDrawFlags flags);
    ...
    IMGUI_API bool
    DataTypeApplyOpFromText(const char *buf, const char *initial_value_buf, ImGuiDataType data_type, void *p_data,
                            const char *format);
